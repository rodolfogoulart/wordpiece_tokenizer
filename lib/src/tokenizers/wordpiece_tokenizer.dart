...
class WordpieceTokenizer {
  ...
  List<int> _tokenizeWord(String word) {
    ...
    while (start < wordLength) {
      ...
      while (start < end) {
        ...
        final token = start == 0 ? _prefixTokens[piece] : _wordpieceTokens[piece];
        ...
        if (end - start == 1 && _isNoSpaceLanguageChar(word.codeUnitAt(start))) {
          tokens.add(unkToken);
          start = end;
          found = true;
          break;
  ...
  List<TextAndTokens> tokenize(String text, {int? maxTokens}) {
    ...
    for (final word in words) {
      ...
      final normalizedWord = _normalizedCache[word] ??= removeDiacritics(word.toLowerCase());
